...
int choose_user_in(void) //bug 输入非法时 提示信息会多出现一次
{
        ...
        switch(choice[0])
    {
        ...
        case '2':system("clear");if(sign_in_driver()) printf("您可以正常登陆\n"); sleep(3);exit(1);
...
int  choose_manager_features()
{
        ...
        switch(choice[0])
    {
        ...
        case '7':choose_sort(phead);print(phead);getchar();free(phead);choose_manager_features();
        case '8':count(phead);free(phead); choose_manager_features();
        case '9':free(phead);choose_user_in() ; 
        case '0':free(phead);exit(1);
        case 'x':system("clear");printf("\n\n\n\n\n\n\n\n\n\n\n\n\t\t\t  请再次确认！！！(y/n)\n");if(getchar() == 'y') {system("rm driver_information");exit(1);}else choose_manager_features();
